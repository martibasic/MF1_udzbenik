```{python}
#| label: fig-uvod-u13
#| fig-cap: "Pregled poglavlja: Cjevovodi"
#| fig-align: center
#| out-width: 95%
...
# --- ZONA 1: Serijsko-paralelna cjevovna mreža ---
ax = ax_fiz
ax.set_xlim(0, 10); ax.set_ylim(0, 8)

# Dovodna cijev (ulaz)
ax.fill([0.3, 2.5, 2.5, 0.3], [3.4, 3.4, 4.6, 4.6], fc=FLUID, ec='#555', lw=1.5, alpha=0.8)
ax.annotate('', xy=(2.0, 4.0), xytext=(0.6, 4.0),
    arrowprops=dict(arrowstyle='->', color=VEL, lw=2.0))
ax.text(1.4, 4.6, r'$Q$', fontsize=11, ha='center', color=VEL)

# Cvor A (razdjelnik)
ax.plot(2.5, 4.0, 'o', ms=10, color='#F39C12', zorder=5)
ax.text(2.5, 4.8, 'A', fontsize=9, ha='center', color='#F39C12')

# Gornja grana
ax.fill([2.5, 7.5, 7.5, 2.5], [4.6, 4.6, 5.4, 5.4], fc=FLUID, ec='#555', lw=1.5, alpha=0.6)
ax.annotate('', xy=(5.5, 5.0), xytext=(3.5, 5.0),
    arrowprops=dict(arrowstyle='->', color=VEL, lw=1.5))
ax.text(5.0, 5.7, r'$Q_1$', fontsize=10, ha='center', color=VEL)

# Donja grana
ax.fill([2.5, 7.5, 7.5, 2.5], [2.6, 2.6, 3.4, 3.4], fc=FLUID, ec='#555', lw=1.5, alpha=0.6)
ax.annotate('', xy=(5.5, 3.0), xytext=(3.5, 3.0),
    arrowprops=dict(arrowstyle='->', color='#8E44AD', lw=1.5))
ax.text(5.0, 2.2, r'$Q_2$', fontsize=10, ha='center', color='#8E44AD')

# Cvor B (spajanje)
ax.plot(7.5, 4.0, 'o', ms=10, color='#F39C12', zorder=5)
ax.text(7.5, 4.8, 'B', fontsize=9, ha='center', color='#F39C12')

# Odvodna cijev
ax.fill([7.5, 9.7, 9.7, 7.5], [3.4, 3.4, 4.6, 4.6], fc=FLUID, ec='#555', lw=1.5, alpha=0.8)
ax.annotate('', xy=(9.2, 4.0), xytext=(8.0, 4.0),
    arrowprops=dict(arrowstyle='->', color=VEL, lw=2.0))

# EGL (pada)
ax.plot([0.3, 2.5], [7.0, 7.0], color='#E74C3C', lw=1.8)
ax.plot([7.5, 9.7], [5.5, 5.5], color='#E74C3C', lw=1.8)
ax.text(0.5, 7.3, 'EGL', fontsize=8, color='#E74C3C')
ax.annotate('', xy=(7.5, 5.5), xytext=(2.5, 7.0),
    arrowprops=dict(arrowstyle='->', color='#E74C3C', lw=1.5, ls='dashed'))
ax.annotate('', xy=(7.5, 5.5), xytext=(2.5, 7.0),
    arrowprops=dict(arrowstyle='->', color='#E74C3C', lw=1.5, ls='dashed'))
ax.annotate('', xy=(2.5, 7.0), xytext=(7.5, 5.5),
    arrowprops=dict(arrowstyle='<->', color='#E74C3C', lw=1.2))
ax.text(5.0, 6.6, r'$h_w = h_{w,1} = h_{w,2}$',
    fontsize=9, ha='center', color='#E74C3C')

# --- ZONA 2: jednadžbe ---
ax = ax_mat
ax.text(0.5, 0.82, r'$h_f = \lambda\,\dfrac{L}{D}\,\dfrac{v^2}{2g}$',
    transform=ax.transAxes, ha='center', va='center', fontsize=13, color=DARK)
ax.text(0.5, 0.50, r'$\text{Serija: } h_{w,uk} = \sum h_{w,i}$',
    transform=ax.transAxes, ha='center', va='center', fontsize=10, color=DARK)
ax.text(0.5, 0.22, r'$\text{Paralela: } h_{w,1} = h_{w,2},\ Q = Q_1 + Q_2$',
    transform=ax.transAxes, ha='center', va='center', fontsize=9.5, color=DARK)

# --- ZONA 3: vodovodna mreža ---
ax = ax_prak
ax.set_xlim(0, 10); ax.set_ylim(0, 6)

# Cvor A
ax.plot(2.0, 3.0, 'o', ms=8, color='#F39C12')
ax.text(1.6, 3.3, 'A', fontsize=9, color='#F39C12')

# Grana 1 (gore, deblja cijev)
ax.fill([2.0, 8.0, 8.0, 2.0], [3.4, 3.4, 4.2, 4.2], fc=FLUID, ec='#555', lw=1.5, alpha=0.7)
ax.text(5.0, 4.6, r'$D_1=150\ mm$', fontsize=8, ha='center', color='#555')

# Grana 2 (dole, tanja cijev)
ax.fill([2.0, 8.0, 8.0, 2.0], [1.8, 1.8, 2.4, 2.4], fc='#AED6F1', ec='#555', lw=1.5, alpha=0.5)
ax.text(5.0, 1.2, r'$D_2=100\ mm$', fontsize=8, ha='center', color='#555')

# Cvor B
ax.plot(8.0, 3.0, 'o', ms=8, color='#F39C12')
ax.text(8.2, 3.3, 'B', fontsize=9, color='#F39C12')

# Strelice protoka
ax.annotate('', xy=(6.0, 3.8), xytext=(4.0, 3.8),
    arrowprops=dict(arrowstyle='->', color=VEL, lw=1.5))
...
```{python}
#| label: fig-u13-paralelne-grane-vodovod
#| fig-cap: "Paralelna vodovodna mreža: D1=150 mm, D2=100 mm, Q=0,050 m3/s, h_w=7,5 m"
#| fig-align: center
#| out-width: 60%
...
# Ulazna cijev (cvor A)
ax.fill([0.5, 2.5, 2.5, 0.5], [3.1, 3.1, 3.9, 3.9],
    fc='#AED6F1', ec='#555', lw=1.8, alpha=0.8)
ax.annotate('', xy=(2.0, 3.5), xytext=(0.8, 3.5),
    arrowprops=dict(arrowstyle='->', color='#27AE60', lw=2.0))
ax.text(1.4, 4.2, r'$Q=0{,}050\ m^3/s$', fontsize=9, ha='center', color='#27AE60')

# Cvor A
ax.plot(2.5, 3.5, 'o', ms=10, color='#F39C12', zorder=5)
ax.text(2.5, 4.4, 'A', fontsize=10, ha='center', color='#F39C12')

# Grana 1 (gore, D1=150mm, deblja cijev)
ax.fill([2.5, 9.5, 9.5, 2.5], [4.2, 4.2, 5.2, 5.2],
    fc='#AED6F1', ec='#555', lw=2.0, alpha=0.8)
ax.annotate('', xy=(7.0, 4.7), xytext=(4.5, 4.7),
    arrowprops=dict(arrowstyle='->', color='#27AE60', lw=2.0))
ax.text(6.0, 5.7, r'$Q_1=37{,}0\ L/s$', fontsize=9, ha='center', color='#27AE60')
ax.text(6.0, 6.2, r'$D_1=150\ mm$,  $L_1=250\ m$', fontsize=8, ha='center', color='#555')

# Grana 2 (dole, D2=100mm, tanja cijev)
ax.fill([2.5, 9.5, 9.5, 2.5], [1.8, 1.8, 2.6, 2.6],
    fc='#AED6F1', ec='#555', lw=1.2, alpha=0.6)
ax.annotate('', xy=(7.0, 2.2), xytext=(4.5, 2.2),
    arrowprops=dict(arrowstyle='->', color='#8E44AD', lw=1.5))
ax.text(6.0, 1.2, r'$Q_2=13{,}0\ L/s$', fontsize=9, ha='center', color='#8E44AD')
ax.text(6.0, 0.5, r'$D_2=100\ mm$,  $L_2=300\ m$', fontsize=8, ha='center', color='#555')

# Cvor B
ax.plot(9.5, 3.5, 'o', ms=10, color='#F39C12', zorder=5)
ax.text(9.5, 4.4, 'B', fontsize=10, ha='center', color='#F39C12')

# Izlazna cijev
ax.fill([9.5, 11.5, 11.5, 9.5], [3.1, 3.1, 3.9, 3.9],
    fc='#AED6F1', ec='#555', lw=1.8, alpha=0.8)
ax.annotate('', xy=(11.2, 3.5), xytext=(10.0, 3.5),
    arrowprops=dict(arrowstyle='->', color='#27AE60', lw=2.0))

# h_w oznaka (isti za obje grane)
ax.text(6.0, 3.4, r'$h_{w,1}=h_{w,2}=7{,}5\ m$',
    fontsize=9.5, ha='center', color='#E74C3C',
    bbox=dict(fc='white', ec='#BDC3C7', boxstyle='round,pad=0.25'))

ax.set_title('Paralelna razvodna mreža vodovoda (T2)', fontsize=9.5, pad=4)
plt.tight_layout()
plt.show()
```
**Rješenje**
